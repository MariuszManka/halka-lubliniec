...
import { ArrowRight, CalendarBlank, List, X } from "@phosphor-icons/react";
      ...
      <header className="home-v2-header" ref={headerRef}>
        ...
        <nav className="home-v2-nav" aria-label="Główna nawigacja">
          {mainNavigation.map((item) => (
            <Link aria-current={item.href === "/historia" ? "page" : undefined} href={item.href} key={item.href}>{item.label}</Link>
          ))}
        </nav>
        ...
        {menuOpen && (
          <nav className="home-v2-mobile-nav" id="history-mobile-menu" aria-label="Menu mobilne">
            {mainNavigation.map((item) => (
              <Link aria-current={item.href === "/historia" ? "page" : undefined} href={item.href} key={item.href}>{item.label}</Link>
            ))}
